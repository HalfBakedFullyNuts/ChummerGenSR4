 ...
 */
function calculateTotalAttributeCost(char: Character): number {
	let total = 0;
	...
	for (const code of ALL_ATTR_KEYS) {
		...
		if (isKarma) {
			/* Karma: sum of (new rating × 5) for each point */
			...
			if (isAtMax && pointsAboveMin > 0) {
				/* All points except the last cost 10, the last costs 25 */
...
export function setAttribute(
	...
	/* Build updated character with new attribute */
