...
<div class="space-y-6">
	...
	<div class="cw-panel p-4">
		<div class="grid grid-cols-3 gap-4 text-center">
			<div>
				<span class="text-text-muted text-xs block">Starting Nuyen</span>
				<span class="text-primary-dark font-mono text-lg">{formatNuyen($startingNuyen)}</span>
			</div>
			<div>
				<span class="text-text-muted text-xs block">Remaining</span>
				<span class="text-primary-dark font-mono text-lg">{formatNuyen($remainingNuyen)}</span>
			</div>
			<div>
				<span class="text-text-muted text-xs block">Essence</span>
				<span class="text-info-dark font-mono text-lg">{$currentEssence.toFixed(2)}</span>
			</div>
		</div>
	</div>
	...
	<section class="min-h-[300px]">
					...
					{#each BP_TO_NUYEN_RATES as rate}
						<button
							class="w-full flex items-center justify-between p-3 rounded transition-colors
								{resourcesBP === rate.bp
									? 'bg-primary-main/20 border border-primary-dark/50'
					...
					{#each filteredWeapons as weapon}
						<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
							<div class="flex-1 min-w-0">
								<span class="text-text-primary block truncate">{weapon.name}</span>
								<span class="text-text-muted text-xs">
									{weapon.category} | DMG: {weapon.damage} | AP: {weapon.ap}
								</span>
							</div>
							<div class="flex items-center gap-2 ml-2">
								<span class="font-mono text-primary-dark text-xs whitespace-nowrap">
			...
			<div class="cw-card">
				...
				<div class="space-y-2 max-h-[400px] overflow-y-auto">
					{#each filteredArmor as armor}
						<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
							<div class="flex-1 min-w-0">
								<span class="text-text-primary block truncate">{armor.name}</span>
								<span class="text-text-muted text-xs">
		...
		{:else if currentTab === 'cyberware'}
			...
			<div class="cw-card">
				...
				<div class="cw-panel p-3 mb-4">
					<span class="text-text-secondary text-sm block mb-2">Select Grade:</span>
					<div class="flex gap-2 flex-wrap">
						{#each gradeOptions as grade}
							...
							>
								{info.label}
								<span class="text-text-muted ml-1">({info.ess} ESS, {info.cost})</span>
							</button>
						{/each}
					</div>
				</div>
				...
				<div class="space-y-2 max-h-[350px] overflow-y-auto">
					{#each filteredCyberware as cyber}
						...
						<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
							<div class="flex-1 min-w-0">
								<span class="text-text-primary block truncate">{cyber.name}</span>
								<span class="text-text-muted text-xs">
			...
			<div class="cw-card">
				...
				<div class="space-y-2 max-h-[350px] overflow-y-auto">
					{#each filteredBioware as bio}
						...
						<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
							<div class="flex-1 min-w-0">
								<span class="text-text-primary block truncate">{bio.name}</span>
								<span class="text-text-muted text-xs">
			...
			<div class="cw-card">
				...
				<div class="space-y-2 max-h-[400px] overflow-y-auto">
					{#each filteredVehicles as vehicle}
						<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
							<div class="flex-1 min-w-0">
								<span class="text-text-primary block truncate">{vehicle.name}</span>
								<span class="text-text-muted text-xs">
			...
			<div class="cw-card">
				...
				<div class="space-y-2 max-h-[400px] overflow-y-auto">
					{#each filteredMartialArts as style}
						...
						<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
							<div class="flex-1 min-w-0">
								<span class="text-text-primary block truncate">{style.name}</span>
								<span class="text-text-muted text-xs">
									Source: {style.source} p.{style.page}
								</span>
							</div>
							<div class="flex items-center gap-2 ml-2">
								<span class="font-mono text-info-dark text-xs whitespace-nowrap">
			...
			<div class="cw-card">
				...
				<div class="space-y-2 max-h-[400px] overflow-y-auto">
					{#each filteredGear as gear}
						<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
							<div class="flex-1 min-w-0">
								<span class="text-text-primary block truncate">{gear.name}</span>
								<span class="text-text-muted text-xs">
									{gear.category}
									{#if gear.rating > 0}
										| Rating: {gear.rating}
		...
		{:else if currentTab === 'lifestyle'}
			...
			<div class="cw-card">
				...
				{#if ownedLifestyle}
					<div class="cw-panel p-3 mb-4 border-primary-dark/50">
						<div class="flex items-center justify-between">
							<div>
								<span class="text-primary-dark font-medium">{ownedLifestyle.name}</span>
								<span class="text-text-muted text-xs block">
				...
				<div class="space-y-2">
					{#each $gameData.lifestyles as lifestyle}
						...
						>
							<div class="text-left">
								<span class="text-text-primary block">{lifestyle.name}</span>
								<span class="text-text-muted text-xs">
			...
			<div class="space-y-4">
							...
							{#each ownedWeapons as weapon}
								<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
									...
									<div class="flex items-center gap-2">
										<span class="text-text-muted text-xs">{formatNuyen(weapon.cost)}</span>
										<button
											class="text-error-dark hover:text-red-400 text-xs"
						...
						<div class="space-y-2">
							{#each ownedArmor as armor}
								<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
									...
									<div class="flex items-center gap-2">
										<span class="text-text-muted text-xs">{formatNuyen(armor.cost)}</span>
										<button
											class="text-error-dark hover:text-red-400 text-xs"
						...
						<div class="space-y-2">
							{#each ownedCyberware as cyber}
								<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
									...
									<div class="flex items-center gap-2">
										<span class="text-text-muted text-xs">{formatNuyen(cyber.cost)}</span>
										<button
											class="text-error-dark hover:text-red-400 text-xs"
						...
						<div class="space-y-2">
							{#each ownedBioware as bio}
								<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
									...
									<div class="flex items-center gap-2">
										<span class="text-text-muted text-xs">{formatNuyen(bio.cost)}</span>
										<button
											class="text-error-dark hover:text-red-400 text-xs"
						...
						<div class="space-y-2">
							{#each ownedVehicles as vehicle}
								<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
									...
									<div class="flex items-center gap-2">
										<span class="text-text-muted text-xs">{formatNuyen(vehicle.cost)}</span>
										<button
											class="text-error-dark hover:text-red-400 text-xs"
					...
					<div class="cw-card">
						...
						<div class="space-y-2">
							{#each ownedMartialArts as art}
								<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
									<div class="flex-1 min-w-0">
										<span class="text-text-primary block truncate">{art.name}</span>
										<span class="text-text-muted text-xs">
											{art.techniques.length} technique{art.techniques.length !== 1 ? 's' : ''}
										</span>
									</div>
									<div class="flex items-center gap-2">
										<span class="text-text-muted text-xs">{MARTIAL_ARTS_COSTS.STYLE} BP</span>
										<button
											class="text-error-dark hover:text-red-400 text-xs"
					...
					<div class="cw-card">
						...
						<div class="space-y-2">
							{#each ownedGear as gear}
								<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
									<div class="flex-1 min-w-0">
										<span class="text-text-primary block truncate">
											{gear.name}
											{#if gear.quantity > 1}
												<span class="text-text-muted">(x{gear.quantity})</span>
											{/if}
										</span>
										<span class="text-text-muted text-xs">{gear.category}</span>
									</div>
									<div class="flex items-center gap-2">
										<span class="text-text-muted text-xs">{formatNuyen(gear.cost * gear.quantity)}</span>
										<button
											class="text-error-dark hover:text-red-400 text-xs"
				...
				{#if ownedLifestyle}
					<div class="cw-card">
						...
						<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
							<div>
								<span class="text-text-primary">{ownedLifestyle.name}</span>
								<span class="text-text-muted text-xs block">
				...
				{#if totalOwned === 0}
					<div class="cw-panel p-8 text-center">
						<p class="text-text-muted">No equipment purchased yet.</p>
	...
	<div class="cw-panel p-4 text-sm">
		<h4 class="text-primary-dark mb-2">Equipment Tips</h4>
