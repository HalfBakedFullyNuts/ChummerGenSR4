...
<div class="cw-card">
	<div class="flex items-center justify-between mb-4">
		<h2 class="text-lg font-medium text-accent-primary">Career Advancement</h2>
		<div class="flex items-center gap-4">
			<span class="text-sm">
				<span class="text-muted-text">Karma:</span>
				<span class="text-accent-cyan font-mono ml-1">{$character?.karma ?? 0}</span>
			</span>
			<span class="text-sm">
				<span class="text-muted-text">Nuyen:</span>
				<span class="text-accent-success font-mono ml-1">{($character?.nuyen ?? 0).toLocaleString()}¥</span>
			</span>
		</div>
	</div>
			...
			<button
				class="px-3 py-1.5 text-sm rounded transition-colors
					{currentTab === tab.id
						...
						: 'text-secondary-text hover:bg-surface-light'}"
	...
	{#if currentTab === 'overview'}
		<div class="space-y-4">
			...
			<div class="p-3 bg-surface rounded">
				<h3 class="text-sm font-medium text-primary-text mb-2">Award Karma</h3>
			...
			<!-- Award Nuyen -->
			<div class="p-3 bg-surface rounded">
				<h3 class="text-sm font-medium text-primary-text mb-2">Award Nuyen</h3>
				<div class="flex gap-2">
					<input
						type="number"
						class="cw-input w-24"
			...
			<div class="grid grid-cols-2 md:grid-cols-4 gap-3">
				<div class="p-3 bg-surface rounded text-center">
					<div class="text-muted-text text-xs">Total Karma</div>
					<div class="text-accent-cyan font-mono text-lg">{$character?.totalKarma ?? 0}</div>
				</div>
				{#if $character?.magic}
					<div class="p-3 bg-surface rounded text-center">
						<div class="text-muted-text text-xs">Initiate Grade</div>
						<div class="text-accent-magenta font-mono text-lg">{$character.magic.initiateGrade}</div>
					</div>
				{/if}
				{#if $character?.resonance}
					<div class="p-3 bg-surface rounded text-center">
						<div class="text-muted-text text-xs">Submersion</div>
						<div class="text-accent-cyan font-mono text-lg">{$character.resonance.submersionGrade}</div>
					</div>
				{/if}
				<div class="p-3 bg-surface rounded text-center">
					<div class="text-muted-text text-xs">Street Cred</div>
					<div class="text-primary-text font-mono text-lg">{$character?.reputation.streetCred ?? 0}</div>
				</div>
			</div>
		</div>
	{/if}
				...
				{#if attr && typeof attr === 'object'}
					<div class="flex items-center justify-between p-2 bg-surface rounded">
						<div class="flex items-center gap-3">
							<span class="text-primary-text font-medium w-20">{ATTR_NAMES[attrKey]}</span>
							<span class="text-accent-primary font-mono">{attr.base + attr.bonus}</span>
							{#if typeof limits === 'object' && limits !== null && 'max' in limits}
								<span class="text-muted-text text-xs">/ {limits.max}</span>
							{/if}
						</div>
						{#if cost !== null}
							<button
						...
						{:else}
							<span class="text-muted-text text-xs">Max</span>
						{/if}
					</div>
				{/if}
			{/each}
	...
	{#if currentTab === 'skills' && $character}
		<div class="space-y-2 max-h-96 overflow-y-auto">
			{#each $character.skills as skill}
				...
				<div class="flex items-center justify-between p-2 bg-surface rounded">
					<div class="flex items-center gap-3">
						<span class="text-primary-text font-medium">{skill.name}</span>
						<span class="text-accent-primary font-mono">{skill.rating}</span>
						{#if skill.specialization}
							<span class="text-muted-text text-xs">({skill.specialization})</span>
						{/if}
					</div>
					{#if cost !== null}
						<button
		...
		<div class="space-y-4">
			...
			<div class="p-3 bg-surface rounded">
				<div class="flex items-center justify-between mb-2">
					<h3 class="text-sm font-medium text-primary-text">
						Initiation Grade: <span class="text-accent-magenta">{$character.magic.initiateGrade}</span>
					</h3>
					{#if initiationCost}
						<button
							class="cw-btn cw-btn-primary text-sm"
			...
			<!-- Metamagics -->
			<div class="p-3 bg-surface rounded">
				<h3 class="text-sm font-medium text-primary-text mb-2">
					Metamagics
					{#if metamagicSlots > 0}
						<span class="text-accent-success text-xs ml-2">({metamagicSlots} slot{metamagicSlots !== 1 ? 's' : ''} available)</span>
					{/if}
				</h3>
				...
				{#if metamagicSlots > 0}
					<div class="flex flex-wrap gap-1">
						{#each METAMAGICS.filter(m => !$character?.magic?.metamagics.includes(m)) as metamagic}
							<button
								class="px-2 py-0.5 bg-surface-light text-secondary-text text-xs rounded hover:bg-accent-magenta/20 hover:text-accent-magenta transition-colors"
	...
	{#if currentTab === 'resonance' && $character?.resonance}
		<div class="space-y-4">
			...
			<div class="p-3 bg-surface rounded">
				<div class="flex items-center justify-between mb-2">
					<h3 class="text-sm font-medium text-primary-text">
						Submersion Grade: <span class="text-accent-cyan">{$character.resonance.submersionGrade}</span>
					</h3>
					{#if submersionCost}
						<button
							class="cw-btn cw-btn-primary text-sm"
			...
			<div class="p-3 bg-surface rounded">
				<h3 class="text-sm font-medium text-primary-text mb-2">
					Echoes
					{#if echoSlots > 0}
						<span class="text-accent-success text-xs ml-2">({echoSlots} slot{echoSlots !== 1 ? 's' : ''} available)</span>
					{/if}
				</h3>
				...
				{#if echoSlots > 0 && $echoesStore}
					<div class="flex flex-wrap gap-1 max-h-32 overflow-y-auto">
						{#each $echoesStore.filter(e => !$character?.resonance?.echoes.includes(e.name)) as echo}
							<button
								class="px-2 py-0.5 bg-surface-light text-secondary-text text-xs rounded hover:bg-accent-cyan/20 hover:text-accent-cyan transition-colors"
	...
	{#if currentTab === 'spirits' && $character?.magic}
		<div class="space-y-4">
			<!-- Add Spirit Form -->
			<div class="p-3 bg-surface rounded">
				<h3 class="text-sm font-medium text-primary-text mb-2">Summon/Bind Spirit</h3>
			...
			<div class="space-y-2">
				{#if $character.magic.spirits.length === 0}
					<p class="text-muted-text text-sm text-center py-4">No spirits summoned.</p>
				{:else}
					{#each $character.magic.spirits as spirit}
						<div class="flex items-center justify-between p-2 bg-surface rounded">
							<div class="flex items-center gap-3">
								<span class="text-primary-text font-medium">{spirit.type}</span>
								<span class="text-accent-magenta text-sm">F{spirit.force}</span>
								<span class="text-muted-text text-sm">{spirit.services} service{spirit.services !== 1 ? 's' : ''}</span>
								{#if spirit.bound}
									<span class="px-1.5 py-0.5 bg-accent-success/20 text-accent-success text-xs rounded">Bound</span>
								{/if}
							</div>
	...
	{#if currentTab === 'sprites' && $character?.resonance}
		<div class="space-y-4">
			<!-- Add Sprite Form -->
			<div class="p-3 bg-surface rounded">
				<h3 class="text-sm font-medium text-primary-text mb-2">Compile Sprite</h3>
				<div class="grid grid-cols-2 md:grid-cols-4 gap-2">
					...
					<div class="flex items-center gap-1">
						<span class="text-xs text-muted-text">R:</span>
						<input type="number" class="cw-input text-sm w-16" bind:value={newSpriteRating} min="1" max="12" />
					</div>
					<div class="flex items-center gap-1">
						<span class="text-xs text-muted-text">Tasks:</span>
			...
			<div class="space-y-2">
				{#if $character.resonance.sprites.length === 0}
					<p class="text-muted-text text-sm text-center py-4">No sprites compiled.</p>
				{:else}
					{#each $character.resonance.sprites as sprite}
						<div class="flex items-center justify-between p-2 bg-surface rounded">
							<div class="flex items-center gap-3">
								<span class="text-primary-text font-medium">{sprite.type}</span>
								<span class="text-accent-cyan text-sm">R{sprite.rating}</span>
								<span class="text-muted-text text-sm">{sprite.tasks} task{sprite.tasks !== 1 ? 's' : ''}</span>
								{#if sprite.registered}
									<span class="px-1.5 py-0.5 bg-accent-success/20 text-accent-success text-xs rounded">Registered</span>
								{/if}
							</div>
							<div class="flex gap-1">
								<button
	...
	{#if currentTab === 'history'}
		<div class="max-h-96 overflow-y-auto">
			{#if expenseLog.length === 0}
				<p class="text-muted-text text-sm text-center py-4">No transactions yet.</p>
			{:else}
				<div class="space-y-1">
					{#each expenseLog as entry}
						<div class="flex items-center justify-between p-2 bg-surface rounded text-sm">
							...
							<span class="font-mono {entry.amount >= 0 ? 'text-accent-success' : 'text-accent-danger'}">
