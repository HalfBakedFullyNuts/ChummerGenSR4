...
<div class="cw-card">
	<div class="flex items-center justify-between mb-3">
		<h3 class="text-primary-text font-medium">Combat Modifiers</h3>
		<div class="flex items-center gap-3">
			<div class="text-sm">
				<span class="text-muted-text">Total:</span>
				<span class="font-mono font-bold {totalModifier < 0 ? 'text-accent-danger' : totalModifier > 0 ? 'text-accent-success' : 'text-primary-text'}">
	...
	<div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
		{#each categories as cat}
			<div class="space-y-1">
				<h4 class="text-xs text-muted-text uppercase tracking-wide">{cat.label}</h4>
				{#each getModifiersByCategory(cat.key) as [key, mod]}
					...
					>
						...
						<span class="float-right font-mono {mod.modifier < 0 ? 'text-accent-danger' : 'text-accent-success'}">
	...
	<div class="border-t border-border pt-3">
		<h4 class="text-xs text-muted-text uppercase tracking-wide mb-2">Called Shots</h4>
		<div class="flex flex-wrap gap-2">
			{#each getApplicableCalledShots() as [key, shot]}
				...
				>
					{shot.name}
					<span class="font-mono text-accent-danger ml-1">{shot.modifier}</span>
				</button>
			{/each}
		</div>
		{#if selectedCalledShot}
			<p class="text-xs text-accent-cyan mt-2">
		...
		<div class="mt-3 pt-3 border-t border-border">
			<div class="text-xs text-muted-text">
				...
				{#each selectedModifiers as mod, i}
					<span class="text-secondary-text">{COMBAT_MODIFIERS[mod].name}</span>{i < selectedModifiers.length - 1 ? ', ' : ''}
				{/each}
				{#if selectedCalledShot}
					{selectedModifiers.length > 0 ? ', ' : ''}
					<span class="text-accent-warning">{CALLED_SHOTS[selectedCalledShot].name}</span>
				{/if}
			</div>
		</div>
	{/if}
