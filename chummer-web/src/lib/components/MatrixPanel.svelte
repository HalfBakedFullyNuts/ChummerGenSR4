...
<div class="cw-card">
	<div class="flex items-center justify-between mb-3">
		<h3 class="text-primary-text font-medium">Matrix Actions</h3>
		<div class="flex items-center gap-3">
			<label class="flex items-center gap-2 cursor-pointer">
				<input
					...
					bind:checked={vrMode}
				/>
				<span class="text-secondary-text text-sm">Hot-Sim VR</span>
			</label>
		</div>
	</div>

	<!-- Commlink Stats -->
	<div class="grid grid-cols-4 gap-2 mb-4 p-2 bg-surface-light rounded">
		<div class="text-center">
			<div class="text-xs text-muted-text">Response</div>
			<div class="font-mono text-accent-cyan">{commlink.response}</div>
		</div>
		<div class="text-center">
			<div class="text-xs text-muted-text">Signal</div>
			<div class="font-mono text-accent-cyan">{commlink.signal}</div>
		</div>
		<div class="text-center">
			<div class="text-xs text-muted-text">Firewall</div>
			<div class="font-mono text-accent-success">{commlink.firewall}</div>
		</div>
		<div class="text-center">
			<div class="text-xs text-muted-text">System</div>
			<div class="font-mono text-primary-text">{commlink.system}</div>
		</div>
	</div>
	...
	<div class="space-y-4">
		{#each actionCategories as category}
			<div>
				<h4 class="text-xs text-muted-text uppercase tracking-wide mb-2">{category.name}</h4>
					...
					{#each category.actions as actionKey}
						...
						>
							<div class="flex items-center justify-between">
								<span class="text-sm text-secondary-text truncate">{action.name}</span>
								<span class="font-mono text-accent-cyan text-sm ml-2">{pool}d6</span>
							</div>
							<div class="flex items-center gap-2 mt-1">
								<span class="text-xs text-muted-text">{action.skill}</span>
								{#if action.opposed}
									<span class="text-xs text-accent-warning" title="Opposed by: {action.opposedBy}">Opp</span>
								{/if}
								{#if action.program}
									<span class="text-xs text-accent-magenta" title="Uses {action.program} program">+{action.program}</span>
								{/if}
							</div>
						</button>
					{/each}
	...
	{#if programs.length > 0}
		<div class="mt-4 pt-3 border-t border-border">
			<h4 class="text-xs text-muted-text uppercase tracking-wide mb-2">Loaded Programs</h4>
			<div class="flex flex-wrap gap-2">
				{#each programs as program}
					<span class="text-xs py-1 px-2 bg-surface-light rounded text-secondary-text">
						{program.name}
						<span class="font-mono text-accent-magenta">R{program.rating}</span>
					</span>
				{/each}
			</div>
		</div>
	...
	<!-- Matrix Defense -->
	<div class="mt-4 pt-3 border-t border-border">
		<h4 class="text-xs text-muted-text uppercase tracking-wide mb-2">Defense Pools</h4>
		<div class="grid grid-cols-2 gap-2 text-sm">
			<div class="flex justify-between p-2 bg-surface-light rounded">
				<span class="text-secondary-text">vs Hacking</span>
				<span class="font-mono text-accent-success">
					{calculateMatrixDefense(commlink.firewall, getProgramRating('Analyze'))}d6
				</span>
			</div>
			<div class="flex justify-between p-2 bg-surface-light rounded">
				<span class="text-secondary-text">vs Cybercombat</span>
				<span class="font-mono text-accent-success">
