...
<div class="cw-card">
	<div class="flex items-center justify-between mb-3">
		<h3 class="text-primary-text font-medium">Technomancer</h3>
	...
	<div class="grid grid-cols-4 gap-2 mb-4 p-2 bg-surface-light rounded text-center">
		<div>
			<div class="text-xs text-muted-text">Compiling</div>
			<div class="font-mono text-accent-cyan">{compilingPool}d6</div>
		</div>
		<div>
			<div class="text-xs text-muted-text">Register</div>
			<div class="font-mono text-accent-cyan">{registeringPool}d6</div>
		</div>
		<div>
			<div class="text-xs text-muted-text">Threading</div>
			<div class="font-mono text-accent-magenta">{threadingPool}d6</div>
		</div>
		<div>
			<div class="text-xs text-muted-text">Fading</div>
			<div class="font-mono text-accent-success">{fadingResistPool}d6</div>
		</div>
	</div>

	<!-- Sprite Compilation -->
	<div class="mb-4">
		<h4 class="text-xs text-muted-text uppercase tracking-wide mb-2">Sprite Compilation</h4>

		<div class="grid grid-cols-2 gap-3 mb-3">
			<div>
				<label class="text-xs text-muted-text block mb-1">Sprite Type</label>
				<select
			...
			<div>
				<label class="text-xs text-muted-text block mb-1">Rating</label>
				<input
					type="number"
					class="cw-input text-sm w-full"
		...
		<!-- Sprite Info -->
		<div class="p-2 bg-surface-light rounded mb-3">
			<div class="text-sm text-secondary-text">{SPRITE_TYPES[selectedSpriteType].name}</div>
			<div class="text-xs text-muted-text mt-1">
				Abilities:
				{#each SPRITE_TYPES[selectedSpriteType].abilities as ability, i}
					<span class="text-accent-cyan">{ability.name}</span>{i < SPRITE_TYPES[selectedSpriteType].abilities.length - 1 ? ', ' : ''}
				{/each}
			</div>
		</div>
		...
		<div class="flex items-center justify-between p-2 bg-surface-light rounded mb-3">
			<div>
				<span class="text-sm text-secondary-text">Your Pool</span>
				<span class="font-mono text-accent-cyan ml-2">{compilingPool}d6</span>
			</div>
			<div>
				<span class="text-xs text-muted-text">Sprite Resists</span>
				<span class="font-mono text-accent-warning ml-1">{calculateSpriteResistance(spriteRating)}d6</span>
			</div>
		</div>
		...
		<p class="text-xs text-muted-text mt-2">
			...
			{#if isFadingPhysical(calculateFadingValue(spriteRating, resonance), resonance)}
				<span class="text-accent-danger">(Physical!)</span>
			{:else}
				<span class="text-accent-warning">(Stun)</span>
			{/if}
		</p>
	</div>
	...
	<div class="border-t border-border pt-3 mb-4">
		<h4 class="text-xs text-muted-text uppercase tracking-wide mb-2">Threading Complex Forms</h4>

		<div class="grid grid-cols-2 gap-3 mb-3">
			<div>
				<label class="text-xs text-muted-text block mb-1">Complex Form</label>
				<select
					class="cw-input text-sm w-full"
					bind:value={formName}
				>
			...
			<div>
				<label class="text-xs text-muted-text block mb-1">Target Rating</label>
				<input
					type="number"
					class="cw-input text-sm w-full"
		...
		<div class="flex items-center justify-between p-2 bg-surface-light rounded mb-3">
			<div>
				<span class="text-sm text-secondary-text">Threading Pool</span>
				<span class="font-mono text-accent-magenta ml-2">{threadingPool}d6</span>
			</div>
			<div>
				<span class="text-xs text-muted-text">Fading Value</span>
				<span class="font-mono {fadingIsPhysical ? 'text-accent-danger' : 'text-accent-warning'} ml-1">
		...
		<p class="text-xs text-muted-text mt-2">
			...
			{#if formRating > resonance}
				<span class="text-accent-danger">Warning: Threading above RES causes Physical fading!</span>
			{/if}
		</p>
	</div>
	...
	<div class="border-t border-border pt-3">
		<h4 class="text-xs text-muted-text uppercase tracking-wide mb-2">Fading Resistance</h4>

		<div class="flex items-center justify-between p-2 bg-surface-light rounded mb-3">
			<div>
				<span class="text-sm text-secondary-text">Resist Pool (RES + WIL)</span>
				<span class="font-mono text-accent-success ml-2">{fadingResistPool}d6</span>
			</div>
		</div>
	...
	<div class="border-t border-border pt-3 mt-4">
		<h4 class="text-xs text-muted-text uppercase tracking-wide mb-2">Quick Reference</h4>
		<div class="text-xs text-muted-text space-y-1">
			<p><span class="text-secondary-text">Tasks from Compiling:</span> Net hits</p>
			<p><span class="text-secondary-text">Services from Registering:</span> Net hits</p>
			<p><span class="text-secondary-text">Threading Duration:</span> Sustaining penalty (-2 per form)</p>
		</div>
	</div>
</div>
