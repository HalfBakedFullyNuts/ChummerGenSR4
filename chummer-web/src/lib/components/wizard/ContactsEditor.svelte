...
<div class="space-y-6">
	...
	<div class="cw-panel p-4">
		<div class="flex items-center justify-between">
			<span class="text-secondary-text">Contacts BP Spent:</span>
			<span class="font-mono text-accent-primary text-xl">{contactsBP}</span>
		</div>
	...
	<div class="cw-card">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
			<!-- Name -->
			<div>
				<label class="block text-secondary-text text-sm mb-1">Name</label>
				<input
					type="text"
			...
			<!-- Type -->
			<div>
				<label class="block text-secondary-text text-sm mb-1">Type</label>
			...
			<!-- Loyalty -->
			<div>
				<label class="block text-secondary-text text-sm mb-1">
					Loyalty: {newLoyalty}
					<span class="text-muted-text">({getLoyaltyDesc(newLoyalty)})</span>
				</label>
				<div class="flex items-center gap-2">
					...
					/>
					<span class="font-mono text-accent-primary w-8 text-center">{newLoyalty}</span>
				</div>
			</div>
			...
			<div>
				<label class="block text-secondary-text text-sm mb-1">
					...
					<span class="text-muted-text">({getConnectionDesc(newConnection)})</span>
				</label>
				<div class="flex items-center gap-2">
					<input
						type="range"
					...
					/>
					<span class="font-mono text-accent-cyan w-8 text-center">{newConnection}</span>
				</div>
			</div>
		</div>
		...
		<div class="flex items-center justify-between">
			<span class="text-muted-text text-sm">
				Cost: <span class="text-accent-primary font-mono">{contactBP(newLoyalty, newConnection)}</span> BP
			</span>
			<button
			...
			<div class="space-y-3">
				{#each contacts as contact}
					<div class="flex items-center gap-4 p-3 bg-surface rounded">
						...
						<div class="flex-1">
							<div class="flex items-center gap-2">
								<span class="font-medium text-primary-text">{contact.name}</span>
						...
						<!-- BP Cost -->
						<div class="text-accent-primary font-mono">
							{contactBP(contact.loyalty, contact.connection)} BP
						</div>

						<!-- Remove Button -->
						<button
							class="w-8 h-8 rounded bg-error/20 text-error hover:bg-error hover:text-background
	...
	{:else}
		<div class="cw-panel p-8 text-center">
			<p class="text-secondary-text mb-2">No contacts yet.</p>
	...
	<div class="cw-panel p-4 text-sm">
		<h4 class="text-accent-primary mb-2">Contact Guidelines</h4>
