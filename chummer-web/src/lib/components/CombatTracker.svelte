...
<div class="cw-card">
	<h2 class="cw-card-header flex items-center justify-between">
		...
		{#if combatActive}
			<span class="text-sm font-normal text-accent-primary">
	...
	<div class="space-y-2 mb-4">
		{#if combatants.length === 0}
			<p class="text-muted-text text-sm">No combatants added.</p>
		{:else}
			{#each (combatActive ? sortedCombatants : combatants) as combatant (combatant.id)}
				...
				<div class="flex items-center justify-between p-2 rounded bg-surface-light
					{combatant.isPlayer ? 'border-l-2 border-accent-primary' : ''}
						...
						<div class="flex flex-col">
							<span class="text-primary-text font-medium">
								...
								{#if combatant.isPlayer}
									<span class="text-accent-primary text-xs">(You)</span>
								{/if}
							</span>
					...
					<div class="flex items-center gap-2">
						{#if combatant.initiative}
							<div class="text-right">
								<div class="font-mono font-bold text-accent-cyan">
						...
						{:else if combatActive && hasActed}
							<span class="text-accent-success text-xs">Done</span>
						{/if}
	...
	{#if !combatActive}
		<div class="border-t border-border pt-4 mb-4">
			<h3 class="text-sm font-medium text-secondary-text mb-2">Add Combatant</h3>
			<div class="flex gap-2 flex-wrap">
				...
				<div class="flex items-center gap-1">
					<label class="text-muted-text text-xs">Init:</label>
					<input
						type="number"
				...
				<div class="flex items-center gap-1">
					<label class="text-muted-text text-xs">Dice:</label>
					<input
						type="number"
	...
	<div class="border-t border-border pt-4 flex gap-2 flex-wrap">
		{#if !combatActive}
			...
			{#if combatants.length > 1}
				<button
					class="cw-btn text-accent-danger"
